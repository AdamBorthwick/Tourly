...
(function () {
  ...
  TourController.prototype.play = function () {
    ...
    // Pan to the correct scroll position for the current time FIRST, then start the video.
    // (When resuming after the user paused and scrolled away, the page glides back before playback.)
    var info = this._activeForTime(this._anchorSeconds);
    var go = function () {
      self._activeIdx = info.idx;
      // Restart dead-reckoning from the frozen position — `_anchorNow` is otherwise stale
      // from the last timeupdate (often many seconds ago if the tour was paused).
      self._anchorNow = now();
      if (self._player && self._playerReady) self._player.play();
      self._setState('playing');
    };
    if (info.y == null || Math.abs(window.scrollY - info.y) < 2) go();
  ...
  TourController.prototype._ensureHiStyles = function () {
    if (document.getElementById('tourly-hi-styles')) return;
    var s = document.createElement('style');
    s.id = 'tourly-hi-styles';
    s.textContent = [
      '.tourly-hi-layer{position:fixed;inset:0;pointer-events:none;z-index:2147482000}',
      '.tourly-hi-overlay{position:fixed;pointer-events:none;border:2px solid var(--tly-hi-color,#ff4d8d);border-radius:4px;box-sizing:border-box;opacity:1}',
      '.tourly-hi-anim-fade-in{animation:tourlyHiFadeIn .45s ease forwards}',
      '@keyframes tourlyHiFadeIn{from{opacity:0}to{opacity:1}}',
      '.tourly-hi-anim-pulse{animation:tourlyHiPulse 1.1s ease-in-out infinite}',
      '@keyframes tourlyHiPulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.72;transform:scale(1.015)}}',
      '.tourly-hi-anim-glow{animation:tourlyHiGlow 1.3s ease-in-out infinite}',
      '@keyframes tourlyHiGlow{0%,100%{box-shadow:0 0 6px 1px var(--tly-hi-color,#ff4d8d)}50%{box-shadow:0 0 20px 6px var(--tly-hi-color,#ff4d8d)}}',
      '.tourly-hi-anim-sweep{animation:tourlyHiSweep 1.8s linear infinite}',
      '@keyframes tourlyHiSweep{0%{box-shadow:-10px 0 0 0 var(--tly-hi-color,#ff4d8d)}25%{box-shadow:0 -10px 0 0 var(--tly-hi-color,#ff4d8d)}50%{box-shadow:10px 0 0 0 var(--tly-hi-color,#ff4d8d)}75%{box-shadow:0 10px 0 0 var(--tly-hi-color,#ff4d8d)}100%{box-shadow:-10px 0 0 0 var(--tly-hi-color,#ff4d8d)}}'
    ].join('');
  ...
  TourController.prototype._layoutHiOverlay = function (h, ov) {
    var node = this._queryTarget(h.target);
    if (!node) { ov.style.display = 'none'; return; }
  ...
  TourController.prototype._syncHighlights = function (t) {
    ...
    for (i = 0; i < active.length; i++) {
      ...
      ov.className = 'tourly-hi-overlay tourly-hi-anim-' + anim;
      if (activeKey !== this._hiActiveKey) ov.style.animation = 'none';
      this._layoutHiOverlay(h, ov);
      if (activeKey !== this._hiActiveKey) { void ov.offsetWidth; ov.style.animation = ''; }
    }
    Object.keys(this._hiOverlays).forEach(function (id) {
      if (!seen[id]) this._hiOverlays[id].style.display = 'none';
    }, this);
